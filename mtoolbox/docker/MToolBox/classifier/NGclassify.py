        ...
        if x != y:
            if x != gap and y != gap:
                ...
                #Trasversione
                    ...
                    mutations.append(mut)
                #Ambiguity
                elif y in list(consts.ambiguity.keys()):
                    if y != 'N':
...
#        self.seq_diff = seq_diff
    def classify90(self):
        ...
        self.haplo_stats90 = OrderedDict(sorted(list(self.haplo_stats90.items()), key=lambda x: x[1][0]/float(x[1][1]), reverse=True))
        # print "haplo_stats90 second stage: ", self.haplo_stats90
        return self.haplo_stats90
    def best_predictions(self):
        self.haplo_stats90_list = sorted(list(self.haplo_stats90.items()), key=lambda x: x[1][0], reverse=True)
    def pprint(self, outfile=sys.stdout):
    ...
    def pprint_sorted(self, outfile=sys.stdout):
        ...
        seq_name = self.seq_diff.obj.name
        seq_snps = len(self.seq_diff.diff_list)
        for haplo in list(self.haplo_stats_sorted.keys()):
    ...
    def prediction_sorting(self):
        ...
        if s == "complete":
            ...
            self.haplo_stats_sorted = OrderedDict(sorted(list(self.haplo_stats.items()), key=lambda x: (-x[1][0], -x[1][0]/float(x[1][1]))))
            # check if there are two haplogroups with max values of Nph and P_Hg
            self.haplo_best = OrderedDict([x for x in list(self.haplo_stats_sorted.items()) if (x[1][0],x[1][0]/float(x[1][1])) == (list(self.haplo_stats_sorted.items())[0][1][0],list(self.haplo_stats_sorted.items())[0][1][0]/float(list(self.haplo_stats_sorted.items())[0][1][1]))])
        else:
            t = "incomplete"
            ...
            if self.classify90():
                ...
                self.haplo_stats_sorted = OrderedDict(sorted(list(self.haplo_stats_sorted.items()), key=lambda x: x[1][0], reverse = True))
            ...
            # cases with no P_Hg >= 0.9, just take the haplogroup(s) with the highest P_Hg
                ...
                # print "got no 90"
                self.haplo_stats_sorted = OrderedDict(sorted(list(self.haplo_stats.items()), key=lambda x: x[1][0]/float(x[1][1]), reverse=True))
                self.haplo_best = OrderedDict([x for x in list(self.haplo_stats_sorted.items()) if x[1][0]/float(x[1][1])])
