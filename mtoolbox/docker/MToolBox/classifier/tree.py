...
def group_equal(pos_list):
    ...
    for idx, pos in enumerate(pos_list[:-1]):
        for dup in filter(pos.__eq__, pos_list[idx+1:]):
...
def group_reverted(pos_list):
    ...
    for idx in reversed(range(len(pos_list)-1)):
        retrom = [x for x in filter(pos_list[idx].is_reverted, pos_list[idx+1:])]
        if retrom:
...
def dict(pos_list):
    ...
    for pos in rev_pos_list:
        if pos.start in list(dict_set_start_list.keys()):
...
def choose_terminal_mutation(dict_set_start_list):
    ...
    for pos_num in list(dict_set_start_list.keys()):
